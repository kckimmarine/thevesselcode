...
check('core sitemap has services hub', coreSitemap.includes('<loc>https://www.thevesselcode.com/services</loc>'));
check('core sitemap no legacy ship-repair-korea url', !coreSitemap.includes('<loc>https://www.thevesselcode.com/ship-repair-korea</loc>'));
...
check('home ship repair korea pillar link', home.includes('href="/services#ship-repair-korea"') && home.includes('Ship Repair in Korea'));
...
check(
    ...
    html.includes('href="https://www.thevesselcode.com/services#ship-repair-korea"')
        && html.includes('Ship Repair in Korea'),
);

const servicesPage = readFileSync(join(root, 'services', 'index.html'), 'utf8');
check('services page ship repair section', servicesPage.includes('id="ship-repair-korea"') && servicesPage.includes('Turnkey Ship Repair &amp; Technical Husbandry in South Korea'));
check('services page meta description ship repair', servicesPage.includes('24/7 ship repair in Korea'));
check('services page port table', servicesPage.includes('srk-port-table') && servicesPage.includes('Busan') && servicesPage.includes('Yeosu'));
check('services page json-ld professional service', servicesPage.includes('"@type": "ProfessionalService"'));
check('services page nationwide banner', servicesPage.includes('srk-nationwide-banner') && servicesPage.includes('/ship-repair-chandler-korea'));
check('services page superintendent cards', servicesPage.includes('id="superintendent-offerings"') && servicesPage.includes('TVC-SM Fleet Integration'));
check('ship repair korea redirect stub', readFileSync(join(root, 'ship-repair-korea', 'index.html'), 'utf8').includes('/services') && readFileSync(join(root, 'ship-repair-korea', 'index.html'), 'utf8').includes('#ship-repair-korea'));
